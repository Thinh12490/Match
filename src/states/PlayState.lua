...
function PlayState:init()
    ...
    self.checkTransitionAlpha = false
...
function PlayState:enter(params)
    ...
    self.board = params.board or Board(VIRTUAL_WIDTH - 272, 16, gameState.level)

    -- grab score from params if it was passed
    self.score = params.score or 0

    -- score we have to reach to get to the next level
    self.scoreGoal = gameState.level * 1.25 * 1000
end
...
function PlayState:update(dt)
    ...
    if self.checkTransitionAlpha then
        Timer.tween(1, {
            [self] = {transitionAlpha = 0}
        })
        :finish(function()
    ...
    if matches then
        ...
        Timer.tween(0.25, tilesToFall):finish(function()
            ...
            if not self.board:checkMatches() then
                ...
                self.canInput = false
                self.checkTransitionAlpha = true              
...
function PlayState:render()
    ...
    love.graphics.printf('Level: ' .. tostring(gameState.level), 20, 24, 182, 'center')
